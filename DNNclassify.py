...
def get_data():
    no_pre_train=[]
    no_pre_test=[]
    no_dnf_floder = ('/Users/wxf/PycharmProjects/DNF/resnet')
    ...
    for i in range(len(no_pre_train)):
        for j in range(len(no_pre_test)):
            if no_pre_train[i].split('_')[0] == no_pre_test[j].split('_')[0]:
                ...
                path = './ORscoreByDNN.csv'

                with open(path, "a+", newline='\n') as file:  # 处理csv读写时不同换行符  linux:\n    windows:\r或者\n    mac:\r
                    csv_file = csv.writer(file)
                    datas = [[no_pre_train[i].split('_')[0] + '_' +no_pre_train[i].split('_')[4].split('.')[0], gmeanave,accave,recallave,f1ave]]
                    # datas = [[no_pre_train[i].split('_')[0] + '_' + no_pre_train[i].split('_')[4] + '_' + no_pre_train[i].split('_')[5].split('.')[0], gmeanave,accave,recallave,f1ave]]
